...
RSpec.describe "Dashboard Tasks Month", type: :request do
  ...
  describe "GET / (dashboard)" do
    ...
    context "when task has status completed" do
      ...
      it "exibe botão verde de entregar (button_to deliver_task_path)" do
        expect(response.body).to include("task_row_#{task_completed.id}")
        expect(response.body).to include("/tasks/#{task_completed.id}/deliver")
        expect(response.body).to include("bg-green-600")
      end

      it "exibe id correto na linha da task" do
        expect(response.body).to include("task_row_#{task_completed.id}")
      end
    end
